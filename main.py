...
async def read_root(request: Request):
    req= await request.json()
    data=pd.read_csv("database.csv")
    data.loc[data["authtoken"]==req["authtoken"],"ememail"]=req["name"]
    data.loc[data["authtoken"]==req["authtoken"],"emphone"]=req["email"]
    data.loc[data["authtoken"]==req["authtoken"],"emname"]=req["mobileNo"]
    data.to_csv("database.csv",index=False)
